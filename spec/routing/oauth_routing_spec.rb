...
RSpec.describe OmniauthCallbacksController, type: :routing do
  ...
  describe 'route to #start' do
    ...
    it { expect(get: "#{admin}/users/auth/twitter").to route_to('omniauth_callbacks#passthru', subdomain: 'admin', provider: 'twitter') }
    it { expect(user_omniauth_authorize_url(:twitter, host: 'domain.com')).to eq "#{admin}/users/auth/twitter" }
  end
